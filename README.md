...
_Inserte la captura de pantalla aquí:_
...
2. **Ciclos de desarrollo y despliegues independientes:** Cuando hay múltiples sub-equipos de desarrollo trabajando sobre diferentes contextos acotados (_Bounded Contexts_). Los microservicios evitan bloqueos en lanzamientos a producción (_releases_).
...
**Respuesta:**
Si se omite el `readinessProbe`, ocurrirá **pérdida de tráfico o peticiones caídas (502 / 503 Bad Gateway)** durante los despliegues o actualizaciones (_Rollouts_).
...
3. **Terminación Controlada (SIGTERM):** Una vez que el pod nuevo está listo, Kubernetes cambia el estado del pod antiguo a `Terminating`, lo remueve del balanceador del Service, y envía una señal `SIGTERM`. Esto le permite al pod antiguo terminar de procesar las conexiones activas (_Graceful Shutdown_) antes de ser destruido.
